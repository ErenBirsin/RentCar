...
export default class Form {
  ...
  readonly showButton = computed(() => {
    if(this.type() === 'pickup' && this.data().reservationStatus === 'Bekliyor')
      return true;
    if(this.type() === 'delivery' && this.data().reservationStatus === 'Teslim Edildi')
      return true;
    return false;
  })
  ...
  onFileSelected(event: Event) {
    ...
    if (input.files && input.files.length > 0) {
      ...
      reader.onload = () => {
        this.images.update(prev => [...prev, reader.result as string]);
      };
