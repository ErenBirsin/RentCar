...
export default class Create {
  ...
  save(form: NgForm) {
    if (!form.valid) {
      this.#toast.showToast("Uyarı", "Lütfen tüm zorunlu alanları doldurun", "error");
      return;
    }

    if(!this.data().customerId){
      this.#toast.showToast("Uyarı", "Müşteri seçmelisiniz", "error");
      return;
    }

    if(!this.data().vehicleId){
      this.#toast.showToast("Uyarı", "Araç seçmelisiniz", "error");
      return;
    }
