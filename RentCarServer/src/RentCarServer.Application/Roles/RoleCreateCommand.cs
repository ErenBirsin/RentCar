...
public sealed class RoleCreateCommandValidator : AbstractValidator<RoleCreateCommand>
{
    ...
    {
        RuleFor(p => p.Name).NotEmpty().WithMessage("Geçerli bir rol adı giriniz");
    }
}

internal sealed class RoleCreateCommandHandler
    (IRoleRepository roleRepository, IUnitOfWork unitOfWork) : IRequestHandler<RoleCreateCommand, Result<string>>
{
    public async Task<Result<string>> Handle(RoleCreateCommand request, CancellationToken cancellationToken)
    {
        var nameExist = await roleRepository.AnyAsync(p => p.Name.Value == request.Name, cancellationToken);
